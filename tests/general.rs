...
fn test_strict_whitespace() {
    ...
    assert_eq!(entries.len(), 1);
    match &entries[0] {
        Entry::Error(e) => assert!(e
            .to_string()
            .contains("Whitespace not allowed between key and equals")),
...
fn test_tab_after_equals_error() {
    ...
    match &entries[0] {
        Entry::Error(e) => assert!(e
            .to_string()
            .contains("Whitespace not allowed after equals")),
        _ => panic!("Should be error"),
    }
}
...
fn test_commented_key_value() {
    ...
    assert_eq!(entries.len(), 2); // Comment + Pair
    assert!(matches!(entries[0], Entry::Comment(_)));
    let kv = entries[1].as_pair().unwrap(); // Changed from entries[0] to entries[1]
    assert_eq!(kv.key, "KEY");
    assert_eq!(kv.value, "value");
...
fn test_mixed_commented_and_active() {
    ...
    // Now returns: Comment, Pair(OLD_KEY), Pair(KEY)
    assert_eq!(entries.len(), 3); // Changed from expecting pairs.len() == 2
    assert!(matches!(entries[0], Entry::Comment(_)));
