...
#[test]
fn test_inline_comment_space() {
    assert_pair("K=v # comment", "K", "v");
}
#[test]
fn test_inline_comment_tab() {
    assert_pair("K=v\t# comment", "K", "v");
}
#[test]
fn test_inline_comment_many_spaces() {
    assert_pair("K=v   # comment", "K", "v");
}
#[test]
fn test_inline_comment_quoted_double() {
    assert_pair("K=\"v\" # comment", "K", "v");
}
#[test]
fn test_inline_comment_quoted_single() {
    assert_pair("K='v' # comment", "K", "v");
}
#[test]
...
#[test]
fn test_hash_no_space() {
    assert_pair("K=val#ue", "K", "val#ue");
}
#[test]
...
#[test]
fn test_hash_in_double_quote() {
    assert_pair("K=\"val # comment\"", "K", "val # comment");
}
#[test]
fn test_hash_in_single_quote() {
    assert_pair("K='val # comment'", "K", "val # comment");
}
...
// "\#" -> \ is unknown escape -> "\#"
#[test]
fn test_hash_escaped_double_check() {
    assert_pair("K=\"\\#\"", "K", "\\#");
}
...
fn test_commented_pair_detection() {
    ...
    // When include_comments is true, we get Comment entry first, then Pair entries inside comment
    assert_eq!(entries.len(), 2);
    assert!(matches!(entries[0], Entry::Comment(_)));
    let pair = entries[1]
        .as_pair()
        .expect("Should be parsed as commented pair");
...
fn test_commented_pair_quoted() {
    ...
    assert_eq!(entries.len(), 2);
    assert!(matches!(entries[0], Entry::Comment(_)));
    let pair = entries[1].as_pair().unwrap();
    assert_eq!(pair.value, "v");
    assert!(pair.is_comment);
}
